    ...
    <div class="bg-white rounded-2xl border-2 border-slate-900 shadow-[4px_4px_0px_0px_#0f172a] p-6 sm:p-8">
        ...
        <form action="{{ route('admin.links.update', $link->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="space-y-3">
                ...
                @if($link->image)
                    <label class="flex items-center gap-2.5 cursor-pointer text-sm font-bold text-slate-700">
                        <input type="checkbox" id="remove_image" name="remove_image" value="1" class="h-4 w-4 rounded border-slate-900 text-rose-600 focus:ring-rose-500">
